...
class Help(commands.Cog, name="Help command"):
    ...
    def get_command_signature(self, command: commands.Command, ctx: commands.Context):
        ...
        if len(command.aliases) > 0:
            ...
            signature = f"{ctx.prefix}{cmd_invoke} {command.signature if command.signature else ''}" if not parent else f"{ctx.prefix}{parent} {cmd_invoke} {command.signature if command.signature else ''}"
            return signature
        else:
            signature = f"{ctx.prefix}{command.name} {command.signature if command.signature else ''}" if not parent else f"{ctx.prefix}{parent} {command.name} {command.signature if command.signature else ''}"
            return signature
